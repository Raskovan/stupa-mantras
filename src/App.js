...
function App() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        let sortedData = [];
        const response = await fetch(process.env.REACT_APP_AWS_DYNAMODB_URI + "&singleEntry=true");
  ...
  useEffect(() => {
    if (
      (allMantras.mantras_count >= 1000000 && allMantras.mantras_count <= 1001000) ||
      (allMantras.mantras_count >= 2000000 && allMantras.mantras_count <= 2001000) ||
      (allMantras.mantras_count >= 3000000 && allMantras.mantras_count <= 3001000) ||
      (allMantras.mantras_count >= 4000000 && allMantras.mantras_count <= 4001000) ||
      (allMantras.mantras_count >= 5000000 && allMantras.mantras_count <= 5001000) ||
      (allMantras.mantras_count >= 6000000 && allMantras.mantras_count <= 6001000) ||
      (allMantras.mantras_count >= 7000000 && allMantras.mantras_count <= 7001000) ||
      (allMantras.mantras_count >= 8000000 && allMantras.mantras_count <= 8001000) ||
      (allMantras.mantras_count >= 9000000 && allMantras.mantras_count <= 9001000) ||
      (allMantras.mantras_count >= 10000000 && allMantras.mantras_count <= 10001000) ||
      (allMantras.mantras_count >= 11000000 && allMantras.mantras_count <= 11005000)
    ) {
  ...
  return (
    <div className="App" style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <Header />
      <MantrasIntro />
      {allMantras.mantras_count >= 0 ? (
        <>
          <Confetti width={width} height={height} recycle={false} run={showConfetti} numberOfPieces={1500} />
